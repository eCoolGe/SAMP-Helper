...
dir = %A_MyDocuments%/GTA San Andreas User Files/SAMP Helper
pver := "1.0.1"                                                                                                ;Версия!
buildscr = %pver% ; Если меньше чем в verlen.ini - программа обновится!
downllen := "https://github.com/eCoolGe/SAMP-Helper/raw/master/verlen.ini"
downlurl := "https://github.com/eCoolGe/SAMP-Helper/raw/master/updt.ahk"
updurl := "https://github.com/eCoolGe/SAMP-Helper/raw/master/SAMP%20Helper.exe"
upddir = %A_MyDocuments%/GTA San Andreas User Files/SAMP Helper/updt.ahk
;####################################################################################
;################################( Автообновление )##################################
;####################################################################################
URLDownloadToFile, %downlurl%, %upddir%
URLDownloadToFile, %downllen%, %dir%/verlen.ini
;####################################################################################
...
WM_HELP(){
    IniRead, vupd, %A_MyDocuments%/GTA San Andreas User Files/SAMP Helper/verlen.ini, UPD, v
    IniRead, updupd, %A_MyDocuments%/GTA San Andreas User Files/SAMP Helper/verlen.ini, UPD, upd
...
SplashTextOn, , 60,Автообновление SA-MP Helper, Пожалуйста подождите! Проверяем наличие обновлений.

IniRead, buildupd, %dir%/verlen.ini, UPD, build
if buildupd =
{
    SplashTextOn, , 60,Автообновление SA-MP Helper, Критическая ошибка! Нет связи с сервером.`nПроверьте ваше соединение с интернетом и попробуйте снова.
    sleep, 2000
}
if buildupd > % buildscr
{
    IniRead, vupd, %dir%/verlen.ini, UPD, v
    SplashTextOn, , 60,Автообновление SA-MP Helper, Обнаружено обновление до версии %vupd%!
    sleep, 2000
    IniRead, desupd, %dir%/verlen.ini, UPD, des
    desupd := Utf8ToAnsi(desupd)
    SplashTextoff
    msgbox, 16384, Автообновление SA-MP Helper, %desupd%
    IfMsgBox OK
    {
        msgbox, 1,Автообновление SA-MP Helper, Хотите ли Вы обновиться до версии %vupd%?
        IfMsgBox OK
        {
            put2 := % A_ScriptFullPath
            RegWrite, REG_SZ, HKEY_CURRENT_USER, Software\SAMP ,put2 , % put2
            SplashTextOn, , 60,Автообновление SA-MP Helper, Обновление. Ожидайте..`nОбновляем скрипт до версии %vupd%!
            URLDownloadToFile, %downlurl%, %dir%/updt.ahk
            sleep, 1000
            run, %dir%/updt.ahk
            exitapp
        }
...
;#####################################( Начало )#####################################
;####################################################################################
MsgBox, SAMP Helper %pver% запущен.`nПрограмма имеет способность сохранять настройки.`n  eCoolGe ©
;####################################################################################
;####################################################################################
Start:
...
if (Car = "ERROR" || Type = "ERROR") {
    FileCreateDir, %A_MyDocuments%\GTA San Andreas User Files\SAMP Helper
    FileAppend,
    (
[INFO]
Car=
...
Settings:
{
    ...
    Gui, Show, x488 y184 h479 w361, SAMP Helper %pver%
}
return
;####################################################################################
Save:
...
BeforeGame:
{
    #IfWinActive, GTA:SA:MP
	{
	 ...
	 addChatMessage("{FF0000}[SAMP Helper] {FFFF00}Версия программы - SAMP Helper " pver ".")
...
{ 
 FileRead, file, %way%
  IfInString, file, Продукт доставлен на склад
   { 
   Sleep 500
   al := al+1
   sum := sum+plus
  ...
  IfInString, file, Рабочий навык увеличен
   {
   Sleep 500
   all := all+1
   FileDelete, %way%
   }
  IfInString, file, Рабочий день завершен
   {
   goto, plantt
   FileDelete, %way%
   }
...
:?:/pinfo:: 
 {
     ...
     addChatMessage("{FF0000}[SAMP Helper] {FFFF00}Всего продуктов: " al " | Рабочий навык: " all " | Заработок: ~" sum ".")
     Sleep 500	 
 }
 return
